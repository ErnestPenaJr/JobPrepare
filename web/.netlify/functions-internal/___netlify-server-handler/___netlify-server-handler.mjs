...
} from '/var/task/projects/JobPrep/web/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer } from '/var/task/projects/JobPrep/web/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/projects/JobPrep/web')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return tracer.withActiveSpan('Next.js Server Handler', async (span) => {
      span.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/projects/JobPrep/web',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/projects/JobPrep/web/.netlify/dist/run/handlers/server.js')
